...
# ── Assemble MEMORY.md ───────────────────────────────────────────────────────
{
  # ① Startup banner — first thing Claude sees, so it outputs it first
  printf '## branch-memory session startup\n\n'
  printf '**Output the following block at the very top of your first response, before anything else:**\n\n'
  printf '```\n'
